...
def timeline(request):
	...
	return render(request, 'my_insta/timeline.html',{"Myprofile":Myprofile,"comment":comment})

@login_required(login_url='/accounts/login/')
def single_pic(request,pic_id):
	pic = pic.objects.get(id= pic_id)

	return render(request, 'my_insta/single_pic.html',{"pic":pic})
...
def send(request):
    ...
    if request.method == 'POST':
        ...
        form = ImageForm() 
    return render(request, 'my_insta/send.html',{"form" : form}) 
